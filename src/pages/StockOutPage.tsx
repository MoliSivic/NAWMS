...
import { CheckCircle, ArrowRight, ScanLine, Package, Clock, AlertTriangle, Loader2, Check } from 'lucide-react';
...
const steps = ['Request Details', 'System Selection (FIFO)', 'Confirm & Dispatch', 'Robot Retrieval', 'Outbound Verification'];

const StockOutPage: React.FC = () => {
  const [step, setStep] = useSessionState('nawms_so_step', 0);
  const [requestType, setRequestType] = useSessionState<'amount' | 'packages' | 'denomination'>('nawms_so_reqType', 'amount');
  const [amount, setAmount] = useSessionState('nawms_so_amount', '250000');
  const [denomReq, setDenomReq] = useSessionState('nawms_so_denomReq', 'USD 100');
  const [pkgCount, setPkgCount] = useSessionState('nawms_so_pkgCount', '0');
  const [submitted, setSubmitted] = useSessionState('nawms_so_submitted', false);
  const [scanList, setScanList] = useSessionState<string[]>('nawms_so_scanList', []);
  const [scanInput, setScanInput] = useState('');
  const [verified, setVerified] = useSessionState('nawms_so_verified', false);

  const [robotStatus, setRobotStatus] = useSessionState<'idle' | 'retrieving' | 'arrived'>('nawms_so_robotStatus', 'idle');
  const [retrievalPhase, setRetrievalPhase] = useSessionState<'idle' | 'moving_to_shelf' | 'loading' | 'returning'>('nawms_so_retrievalPhase', 'idle');
  const [countdown, setCountdown] = useSessionState<number | null>('nawms_so_countdown', null);
  ...
  const confirmRelease = () => {
    ...
    selectedPkgs.forEach(pkg => {
        const originalPkg = mockPackages.find(p => p.packageId === pkg.packageId);
        if (originalPkg) {
        ...
        if (pallet) {
             pallet.currentPackageCount = Math.max(0, pallet.currentPackageCount - 1);
             if (pallet.currentPackageCount === 0) {
                 pallet.status = 'available';
                 
                 for (const shelf of warehouseLayout.shelves) {
                    const slot = shelf.slots.find(s => s.locationId === pallet.locationCode);
    ...
    <div className="min-h-[calc(100vh-120px)] flex flex-col items-center justify-start p-4 py-12">
      <div className="w-full max-w-4xl space-y-6">
        ...
        <div className="bg-card rounded-lg border p-6 animate-fade-in w-full shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 mb-4">
                {(['amount', 'packages', 'denomination'] as const).map(t => (
                  <button key={t} onClick={() => setRequestType(t)} className={`px-3 py-1.5 rounded border text-xs font-medium capitalize ${requestType === t ? 'border-primary bg-primary/5 text-primary' : 'border-border text-muted-foreground'}`}>
                    By {t}
                  </button>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" min="1" placeholder="0" value={pkgCount === '0' || pkgCount === '' ? '' : pkgCount} onChange={e => setPkgCount(e.target.value)} className="w-full px-3 py-1.5 border rounded text-sm bg-background [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                </div>
                <div>
          ...
          {step === 1 && (
            <div className="space-y-4 animate-in slide-in-from-right-2 duration-300">
                ...
                <table className="w-full text-xs">
                  <thead className="bg-muted">
                    <tr className="border-b">
                      <th className="text-left py-2 px-3 text-muted-foreground font-medium">Package ID</th>
                      <th className="text-left py-2 px-3 text-muted-foreground font-medium">Location</th>
                      <th className="text-left py-2 px-3 text-muted-foreground font-medium">Pallet</th>
                      <th className="text-left py-2 px-3 text-muted-foreground font-medium">Arrival</th>
                      <th className="text-left py-2 px-3 text-muted-foreground font-medium">Denominations</th>
                  ...
                  <tbody>
                    {selectedPkgs.map(pkg => (
                      <tr key={pkg.packageId} className="border-b hover:bg-muted/30">
                        <td className="py-2 px-3 font-mono font-medium">{pkg.packageId}</td>
                        <td className="py-2 px-3">{pkg.locationCode}</td>
                        <td className="py-2 px-3 text-muted-foreground">{pkg.palletId}</td>
                        <td className="py-2 px-3">{new Date(pkg.arrivalDate).toLocaleDateString()}</td>
                        <td className="py-2 px-3">
                          {pkg.denominations.map((d, i) => (
                            <span key={i} className="inline-block mr-1 px-1.5 py-0.5 bg-background border rounded text-[10px]">
                              {d.currency} {d.denomination} x{d.quantity}
                            </span>
                          ))}
                        </td>
                        <td className="py-2 px-3 text-right font-medium">{pkg.currency} {pkg.totalValue.toLocaleString()}</td>
                      </tr>
                    ))}
                    {selectedPkgs.length === 0 && (
                      <tr>
                        <td colSpan={6} className="py-8 text-center text-muted-foreground">No matching packages found for criteria.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="p-3 bg-navy-50 border border-navy-100 rounded flex justify-between items-center text-primary">
                ...
                <span className="text-lg font-bold">USD {totalSelectedValue.toLocaleString()}</span>
              </div>
              ...
              <div className="p-3 bg-blue-50/50 border border-blue-100/50 rounded text-xs text-blue-700 flex items-start gap-3">
                ...
                <div>
                  <p className="font-bold uppercase tracking-wider text-[10px]">Estimated Robot Retrieval Route</p>
                  <p className="mt-1 font-medium">
                    {selectedPkgs.map(p => p.locationCode).filter((v, i, a) => a.indexOf(v) === i).join(' → ')} → Outbound Door
                  </p>
                  <p className="opacity-70 mt-0.5">Estimated retrieval time: ~{Math.max(5, selectedPkgs.length * 2)} minutes for {selectedPkgs.length} packages</p>
                </div>
              </div>

              <div className="flex justify-between pt-2">
                <Button size="sm" variant="outline" onClick={() => setStep(0)}>Back</Button>
                <Button size="sm" onClick={() => setStep(2)} disabled={selectedPkgs.length === 0}>Review & Confirm <ArrowRight className="w-3 h-3 ml-1" /></Button>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4 animate-in slide-in-from-right-2 duration-300">
              <h2 className="text-sm font-medium">Confirm & Dispatch Workflow</h2>
              
              <div className="p-4 bg-muted/50 rounded border space-y-3">
                 <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Dispatch Summary</h3>
                 <div className="grid grid-cols-2 gap-4">
                    <div className="bg-background p-3 rounded border shadow-sm">
                      <p className="text-[10px] text-muted-foreground uppercase">Packages to Verify</p>
                      <p className="text-2xl font-bold font-mono text-foreground">{selectedPkgs.length}</p>
                    </div>
                    <div className="bg-background p-3 rounded border shadow-sm">
                      <p className="text-[10px] text-muted-foreground uppercase">Total Outbound Value</p>
                      <p className="text-2xl font-bold font-mono text-primary">USD {totalSelectedValue.toLocaleString()}</p>
                    </div>
                 </div>
                 <p className="text-xs text-muted-foreground">Please confirm the request before dispatching the automated retrieval sequence. Ensure Outbound Area is clear for robot drop-off.</p>
              </div>

              <div className="flex justify-between pt-2">
                <Button size="sm" variant="outline" onClick={() => setStep(1)}>Back</Button>
                <Button size="sm" onClick={handleStartDispatch} className="bg-primary hover:bg-primary/90 text-primary-foreground">
                  Confirm & Dispatch Robot <ArrowRight className="w-3 h-3 ml-1" />
                </Button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-6 animate-in slide-in-from-right-2 duration-300">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-medium">Robotic Retrieval Active</h2>
                {robotStatus === 'retrieving' && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50/80 border border-blue-200 text-blue-700 text-[10px] font-bold rounded-full animate-pulse">
                    ...
                    {retrievalPhase === 'moving_to_shelf' ? `Robot Dispatching to ${selectedPkgs[0]?.locationCode || 'Target'}` : 
                     retrievalPhase === 'loading' ? `Loading pallet at ${selectedPkgs[0]?.locationCode || 'Target'}...` :
              ...
              {robotStatus === 'retrieving' && (
                 <div className="p-4 bg-muted/30 border rounded-lg">
                    <p className="text-xs text-muted-foreground mb-4">The automated system is currently fetching the selected packages. Please wait for the robot to complete its route on the warehouse map below.</p>
                 </div>
              )}
              ...
              {robotStatus === 'arrived' && (
                <div className="p-6 bg-success/5 border border-success/30 rounded-xl flex flex-col items-center justify-center space-y-4 animate-in zoom-in-95 duration-500">
                   ...
                   <div className="text-center">
                     <h3 className="text-lg font-bold text-success mb-1">Payload Arrived</h3>
                     <p className="text-xs text-muted-foreground">The robot has delivered all queried pallets to the Outbound Door.</p>
                   </div>
                   <Button size="sm" onClick={() => setStep(4)} className="w-full max-w-xs mt-2">Proceed to Outbound Verification <ArrowRight className="w-3 h-3 ml-2" /></Button>
                </div>
              )}
          ...
          {step === 4 && (
            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <p className="text-xs text-muted-foreground">Scan each outgoing package at the warehouse exit to verify against the approved request. Packages must match exactly.</p>
              ...
              <div className="p-5 bg-muted/50 rounded-lg border-2 border-dashed border-primary/30 flex flex-col items-center sm:flex-row sm:items-start gap-4">
                ...
                <div className="flex-1 w-full space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Barcode Input</span>
                    <span className="text-xs font-bold text-primary">{scanList.length} / {selectedPkgs.length} SCANNED</span>
                  </div>
                  <div className="flex gap-2">
                    <input value={scanInput} onChange={e => setScanInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleScan()} placeholder="Scan package QR or ID..." className="flex-1 px-4 py-2 border rounded-md text-sm bg-background font-mono shadow-inner" disabled={verified || allScanned} autoFocus={!allScanned} />
                    <Button size="sm" onClick={handleScan} disabled={verified || allScanned || !scanInput.trim()}>Scan/Enter</Button>
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs">
                <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                  {selectedPkgs.map(pkg => {
                    const scanned = scanList.includes(pkg.packageId) || scanList.includes(`NBC-${pkg.packageId}`);
                    return (
                      <div key={pkg.packageId} className={`flex items-center gap-3 px-4 py-2.5 rounded-md border transition-all ${scanned ? 'bg-green-50 border-success/20' : 'bg-background hover:bg-muted/50 border-border'}`}>
                        {scanned ? <CheckCircle className="w-4 h-4 text-success" /> : <Package className="w-4 h-4 text-muted-foreground/50" />}
                        <div className="flex-1">
                           <span className={`font-mono font-bold block ${scanned ? 'text-success' : 'text-foreground'}`}>{pkg.packageId}</span>
                           <span className="text-[10px] text-muted-foreground">Pallet: {pkg.palletId} | Value: {pkg.currency} {pkg.totalValue.toLocaleString()}</span>
                        </div>
                        <span className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-widest ${scanned ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}>
              ...
              {!verified && !allScanned && scanList.length > 0 && (
                <div className="p-3 bg-amber-50 border border-warning/20 rounded text-xs flex items-center gap-2 animate-in fade-in">
                  ...
                  <span>Missing items! All {selectedPkgs.length} packages must be verified before release. {selectedPkgs.length - scanList.length} remaining.</span>
                </div>
              )}

              {!verified && allScanned && (
                <Button size="lg" onClick={confirmRelease} className="w-full bg-success hover:bg-success/90 text-white font-bold h-12 animate-in slide-in-from-bottom-2">
              ...
              {verified && (
                <div className="p-6 bg-green-50 border border-success/30 rounded-xl space-y-4 animate-in zoom-in-95 duration-500 flex flex-col items-center text-center">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-success mb-1">Outbound Operation Complete</h3>
                    <p className="text-xs text-muted-foreground">Verification successful. Packages have been released, locations cleared, and dispatch logged in audit trail.</p>
                  </div>
                  <Button variant="outline" size="sm" onClick={clearSession} className="mt-2 w-full max-w-xs">Start New Outbound Request</Button>
                </div>
              )}
